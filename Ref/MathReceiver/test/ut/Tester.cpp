...
namespace Ref {
  ...
  void Tester ::
    testSub()
  {
    // Set the factor by loading parameters
    const F32 factor = pickF32Value();
    ...
    testThrottle()
  {
    ...
    // Use the autocoded value so the unit test passes if the throttle value is changed
    const F32 factor = pickF32Value();
    for (
      U16 cycle = 0;
    ...
    )
  {
    ...
    // invoke scheduler port to dispatch message
    const U32 context = STest::Pick::any();
    this->invoke_to_schedIn(0, context); // [NOTE] using schedin instead of dispatch because this component is queued
